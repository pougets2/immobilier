...
Frais = [ "Frais" if "fees" in str(annonce.find('p')).lower() else "NA" for annonce in annonces]
...
Frais_non_enchère = ["autre" if Frais[index] == "Frais" and element != "Enchères" else "NA" for index, element in
                     enumerate(Auction)]
...
print("Frais=",len(Frais))
print("Frais_sans_enchères=",len(Frais_non_enchère))
...
immobiler_10 = pd.DataFrame(
   {"URL" : les_urls,
    ...
    "Frais sans enchère" : Frais_non_enchère,
     })
